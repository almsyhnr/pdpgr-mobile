...
class TambahPengajuanBaririNelayan extends ValidationComponent {
  ...
  onSavePress = () => {
    this.validate({
      ...
      jenis_nelayan: { required: true },
      bantuan_nelayan: {required: true}
    })
    console.tron.error(this.state.form)
    console.tron.error(this.getErrorMessages())
    if (this.isFormValid()) {
      if (this.state.form.bantuan_nelayan != null && this.state.form.jenis_nelayan != null) {
      ...
      <View style={styles.container}>
        <KeyboardAwareScrollView style={styles.container}>
          ...
          <View style={{marginTop: 30, backgroundColor: Colors.snow, padding: 20}}>
            ...
            <Divider />
            <Text style={styles.label}>Jenis Nelayan</Text>
            <RNPickerSelect
              placeholder={{
                label: 'Pilih Jenis Nelayan',
                value: null
              }}
              items={this.props.jenisNelayan}
              onValueChange={(value) => {
                this.onValueChange('jenis_nelayan', value)
                // update jenis bantuan
                const bantuanNelayan = _.filter(this.props.bantuanNelayan, item => {
                  if (item.jenis_nelayan === value) return item
                })
                this.setState({
                  bantuanNelayan
                })
              }}
              value={this.state.bantuan_nelayan}
            ...
            <RNPickerSelect
              ...
              items={this.state.bantuanNelayan}
              onValueChange={(value) => {
                this.onValueChange('bantuan_nelayan', value)
              }}
              value={this.state.bantuan_nelayan}
              style={{ ...pickerSelectStyles }}
                />
...
const mapStateToProps = state => {
  const jenisNelayan = _.map(state.masterData.jenisNelayan, item => {
    return {
      label: item.name,
      value: item.id
    }
  ...
  const bantuanNelayan = _.map(state.masterData.bantuanNelayan, item => {
    return {
      label: item.name,
      value: item.id,
      jenis_nelayan: item.jenis_nelayan
    }
  })
  ...
  return {
    ...
    bantuanNelayan: bantuanNelayan,
    jenisNelayan: jenisNelayan
  }
}
