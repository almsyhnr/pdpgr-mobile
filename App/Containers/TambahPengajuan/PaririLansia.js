...
class TambahPengajuanRehabRumah extends ValidationComponent {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      images: {
        ktp: null,
        kk: null,
        suratPernyataanKeluarga: null,
        suratKeteranganLahir: null
      },
      modul: _module,
      subVillages: []
    }
    this.selectedImage = 'ktp'
  }
  ...
  onSavePress = () => {
    ...
    if (this.isFormValid()) {
      ...
      if (!images.ktp || !images.kk || !images.suratPernyataanKeluarga) {
        return Alert.alert('Error', 'Semua field wajib diisi')
      }
      return this.props.createSubmission(form, [images.ktp, images.kk, images.suratPernyataanKeluarga, images.suratKeteranganLahir])
    }
          ...
          <View style={{marginTop: 30, backgroundColor: Colors.snow, padding: 20}}>
            ...
            <View style={styles.customFieldBox}>
              ...
              {!images.ktp ? (<Button
                ...
                onPress={() => this.openImageModal('ktp')} />)
                : this.renderImageItem('ktp', images.ktp)}
            </View>
            <View style={styles.customFieldBox}>
              <Text style={styles.label}>KK *</Text>
              {!images.kk ? (<Button
                title='Pilih File'
                buttonStyle={styles.btnPilihFile}
                disabled={this.props.posting}
                containerStyle={{ marginTop: 20 }}
                onPress={() => this.openImageModal('kk')} />)
                : this.renderImageItem('kk', images.kk)}
            </View>
            <View style={styles.customFieldBox}>
              <Text style={styles.label}>Surat Pernyataan Keluarga *</Text>
              {!images.suratPernyataanKeluarga ? (<Button
                title='Pilih File'
                buttonStyle={styles.btnPilihFile}
                disabled={this.props.posting}
                containerStyle={{ marginTop: 20 }}
                onPress={() => this.openImageModal('suratPernyataanKeluarga')} />)
                : this.renderImageItem('suratPernyataanKeluarga', images.suratPernyataanKeluarga)}
            </View>
            <View style={styles.customFieldBox}>
              <Text style={styles.label}>Surat Keterangan Lahir</Text>
              {!images.suratKeteranganLahir ? (<Button
                title='Pilih File'
                buttonStyle={styles.btnPilihFile}
                disabled={this.props.posting}
                containerStyle={{ marginTop: 20 }}
                onPress={() => this.openImageModal('suratKeteranganLahir')} />)
                : this.renderImageItem('suratKeteranganLahir', images.suratKeteranganLahir)}
            </View>
          </View>
          <View>
            <Button
